...
class Stage extends Container {
  ...
  getClosestAliveDuck(point = null) {
    ...
    for (let i = 0; i < this.ducks.length; i++) {
      ...
      const distance = point ? Utils.pointDistance(duck.position, point) : duck.position.y;
  ...
  shotsFiredAtPrediction(prediction, radius) {
    ...
    const maxLockDistance = Math.max(radius * 3, 140);
    const targetDuck = predictedDuck && predictedDistance <= maxLockDistance ? predictedDuck : fallbackDuck;
